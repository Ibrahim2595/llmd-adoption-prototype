...
description: "Running inference asynchronously for throughput-optimized batch workloads."
sidebar_order: 8
---
...
1. **Kubernetes cluster**: A running Kubernetes cluster (v1.31+). For local development, you can use Kind or Minikube. For production, GKE, AKS, or OpenShift are supported.
...
3. **llm-d Inference Stack**: Async Processor requires an existing [Intelligent Inference Scheduling](/docs/production-deployment/inference-scheduling) stack to dispatch requests to.
...
If your Inference Gateway is deployed elsewhere, export the variable before running helmfile:
...
Testing instructions vary depending on the chosen queue implementation. Please refer to the specific implementation guide:
